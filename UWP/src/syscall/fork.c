...
#elif defined(_M_X64)
	mm_write_process_memory(info.hProcess, (PVOID)ctx.Rsp, (PVOID)ctx.Rsp,
		(SIZE_T)((char *)stack_base - (char*)ctx.Rsp));
#elif defined(_M_IX86)
...
#elif defined(_M_X64)
	ThreadContext.Rsp = (((DWORD64)child_stack) - 0x20) & ~0xFULL;
	ThreadContext.Rip = (DWORD64)__bionic_thread_callback;
	ThreadContext.Rcx = (DWORD64)info;
#elif defined(_M_IX86)
...
static pid_t fork_thread(struct syscall_context *context, void *child_stack, unsigned long flags, void *ptid, void *ctid)
{
	...
	info->context.sp = (uintptr_t)child_stack;
	info->pid = pid;
	info->ctid = ctid;
	info->flags = flags;
	if (flags & CLONE_CHILD_SETTID)
...
static pid_t fork_process_best_effort(struct syscall_context *context, unsigned long flags, void *ptid, void *ctid)
{
	pid_t pid = fork_process(context, flags, ptid, ctid);
	if (pid != -1)
		return pid;

	log_warning("fork(): primary CreateProcess-backed path failed, trying RtlCloneUserProcess fallback.");
...
{
	log_info("fork()");
	return fork_process_best_effort(context, 0, NULL, NULL);
}

int sys_vfork_imp(struct syscall_context *context)
{
	log_info("vfork()");
	return fork_process_best_effort(context, CLONE_VFORK, NULL, NULL);
}
...
#endif
{
	...
	if (flags & CLONE_THREAD)
		...
		return fork_process_best_effort(context, flags, ptid, ctid);
